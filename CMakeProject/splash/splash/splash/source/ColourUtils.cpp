...
namespace Splash::ColourUtils {
    ...
    Colour changeColourLightness(Colour& base, int amount) {
        ...
        lab.l = std::max(std::min(100.0, lab.l + amount), 0.0);
    ...
    Colour findContrastColour(Colour& col, Colour& other, bool findFg, double ratio) {
        ...
        double a = lab.a;
        double b = lab.b;
        for (size_t i = 0; i < 15 && (high - low) > 0.00001; i++) {
    ...
    double pivotXyzComponent(double component) {
        return (component > XYZ_EPSILON ? std::pow(component, 1 / 3.0) : (XYZ_KAPPA * component + 16) / 116.0);
    }
    ...
    Colour XYZToColour(XYZ& xyz) {
        double r = (xyz.x * 3.2406 + xyz.y * -1.5372 + xyz.z * -0.4986) / 100.0;
        double g = (xyz.x * -0.9689 + xyz.y * 1.8758 + xyz.z * 0.0415) / 100.0;
        double b = (xyz.x * 0.0557 + xyz.y * -0.2040 + xyz.z * 1.0570) / 100.0;
    ...
    LAB XYZToLAB(XYZ xyz) {
        ...
        lab.l = std::max(0.0, 116 * xyz.y - 16);
    ...
    XYZ colourToXYZ(Colour& c) {
        ...
        out.x = 100.0 * (sr * 0.4124 + sg * 0.3576 + sb * 0.1805);
        out.y = 100.0 * (sr * 0.2126 + sg * 0.7152 + sb * 0.0722);
        out.z = 100.0 * (sr * 0.0193 + sg * 0.1192 + sb * 0.9505);
